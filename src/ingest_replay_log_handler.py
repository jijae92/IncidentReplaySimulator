...
def lambda_handler(event: Dict[str, Any], context: LambdaContext) -> Dict[str, Any]:
    ...
    for i in range(count):
        ...
        try:
            ...
            generated_objects.append(f"s3://{config.bucket_replay}/{s3_key}")
            logger.info(f"Successfully ingested replay log to s3://{config.bucket_replay}/{s3_key}")
